...
public class TargetSum {
    ...
    private int helper2(int[] nums, int current, int sum, int target) {
        if (current == nums.length) return sum == target ? 1 : 0;
        if (mem[current][sum + total] != -1) return mem[current][sum + total];
        return mem[current][sum + total] = helper2(nums, current + 1, sum + nums[current], target)
                + helper2(nums, current + 1, sum - nums[current], target);
    }

    public int findTargetSumWaysMem(int[] nums, int S) {
        for (int num : nums) total += num;
        mem = new int[nums.length][total * 2 + 1];
        for (int[] row : mem) Arrays.fill(row, -1);
        return helper2(nums, 0, 0, S);
    }
